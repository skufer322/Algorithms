...
public class GreedyInjectionModule extends AbstractModule {
    ...
    protected void configure() {
        ...
        bind(MstAlg.class).annotatedWith(Names.named(GreedyConstants.INJECTION_NAME_MST_PRIMS_ALG_HEAP_BASED)).to(PrimsAlgHeapBased.class);
        bind(MstAlg.class).annotatedWith(Names.named(GreedyConstants.INJECTION_NAME_MST_KRUSKAL_ALG_UNION_FIND_BASED)).to(KruskalAlgUnionFindBased.class);

        // bind union-find implementations
        bind(UnionFind.class).to(UnionFindSizeBased.class);
        bind(UnionFind.class).annotatedWith(Names.named(GreedyConstants.INJECTION_NAME_UNION_FIND_BY_SIZE)).to(UnionFindSizeBased.class);
